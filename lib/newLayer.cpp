...
public:
    ...
    void adjust() {
    	std::vector<Line> clippingEdges = clippingPolygon->getLines();
    	std::vector<Polygon> clippedPolygons;
    	for (std::vector<Polygon>::iterator polygon = polygons->begin(); polygon != polygon->end(); ++polygon) {
    		for (std::vector<Line>::iterator edge = clippingEdges.begin(); edge != clippingEdges.end(); ++edge) {
	    		for (int it = 0; it < polygon->size(); ++it) {
	    			Point pi = polygon[i];
	    			Point pinext = polygon[i+1];

	    			if (isPointInsidePolygon(pi, )) {
