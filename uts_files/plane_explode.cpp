...
void PlaneExplode() {
	...
	while (true) {
		...
		// pesawat & kapal
		plane.draw(&fb);
		plane.drawFill(&fb, &planepattern);
		//ship.draw(&fb);
		...
		if (periodeKapal < 0) {
			...
			periodeKapal = 1000.0;
			peluruKapal.p.x = 400;// + (ship.getBottomRight().x - ship.getTopLeft().x) / 2;
			peluruKapal.p.y = 600;
		}
		if (existPeluruKapal)
	...
	for (int i = 1; i <= nPieces; ++i) {
		Sprite piece(planecolor, Color::BLUE);
		char temp[100];
	...
	// ground limit
	int groundY = 600; // + ship.getSize().y;
	...
	while (true) {
		...
		for (int i = 0; i < nPieces; ++i) {
			...
			tempPiece.draw(&fb);
			tempPiece.drawFill(&fb, &planepattern);
		}
