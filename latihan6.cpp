	...
	while (true) {
			...
			if (prevleft == 0) {
				if (ship.collide(canvas.viewToWindow(axis))) {
					...
					if (!isShipExplode){
						...
						Point dummyShipBulletV = canvas.viewToWindow(axis) - ship.getPos();
		...
		if(shipBulletexist){
			...
			if(plane.collide(shipBullet.getPos())){
				...
				canvas.removeSprite(plane);
				minimap.removeSprite(shipBullet);
				minimap.removeSprite(plane);
			}
		}
		...
		if(!planeBulletexist && !isPlaneExplode){
			...
			planeBulletT = 0;
			canvas.addSprite(&planeBullet);
			minimap.addSprite(&planeBullet);
		}
		...
		if(planeBulletexist){
			...
			if(ship.collide(planeBullet.getPos())){
				...
				canvas.removeSprite(ship);
				minimap.removeSprite(planeBullet);
				minimap.removeSprite(ship);
			}
		}
